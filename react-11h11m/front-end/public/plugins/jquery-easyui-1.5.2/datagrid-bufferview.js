...
var bufferview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	onBeforeRender: function(target, rows){
		var buffer_options = $(target).datagrid('options');
		buffer_options.firstCall = true;
		$(target).datagrid('resize', { height: rows.length == 0 ? buffer_options.emptyHeight : 'auto'});
		/*if(buffer_options.dialogId) {
			$('#' + buffer_options.dialogId).dialog('center').dialog('resize');
		}*/
		var state = $.data(target, 'datagrid');
		var opts = state.options;
		var dc = state.dc;
		...
		function scrolling(){
			...
			} else if (dc.body2.scrollTop() >= getDataHeight() - dc.body2.height()){	//스크롤이 끝지점에 다다랐을때 실행되는 직접 로직
				this.getRows.call(this, target, function(rows){
					rows.forEach(function(row) {	//bufferIndex 값이 필요한 경우가 발생
						row['bufferIndex'] = opts.bufferIndex++;
					});
					this.rows = rows;
					this.populate.call(this, target);
				});
...
//			return view.renderedCount * opts.rowHeight;
	...
	getRows: function(target, callback){
		var state = $.data(target, 'datagrid');
		var opts = state.options;
		var page = Math.floor(this.renderedCount/opts.pageSize);
		
		if(opts.paginationBufferId){
			$('#' + opts.paginationBufferId).epeBufferPagination('pageRefresh', page);
		}
...
//		var rows = state.data.rows.slice(index, index+opts.pageSize);
		if (rows.length){
			...
			var param = $.extend({}, opts.queryParams, {
				page: Math.floor(this.renderedCount/opts.pageSize),
				rows: opts.pageSize,
				size: opts.pageSize
			});
			if (opts.sortName){
	...
	populate: function(target){
		...
		if (this.rows.length){
			...
			if(opts.firstCall && opts.autoMaxHeight){ //cc.yhlee
				var pageRow = Boolean(opts.pageRow) ? Number(opts.pageRow) : 10;
				if(this.renderedCount >= pageRow){
					 ...
					 $(target).datagrid('resize', { maxHeight: ++maxHeightValue });
